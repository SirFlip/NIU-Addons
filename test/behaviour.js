...
(async () => {
  ...
  const enc = probe('encodeLatin1');
  t('encodeLatin1: Umlaute als Latin-1, Sonderzeichen ersetzt', enc('Grüße & Co, 5€ – „ok“') === 'Gr%FC%DFe%20%26%20Co%2C%205EUR%20-%20%22ok%22' && enc(undefined) === '' && enc('日') === '%3F');
  const kl = probe('kommandoLinks')('8123', { EID: 'e', ENID: 'n' }, true);
  t('kommandoLinks: aktueller Host, DNr und IDs eingesetzt', /href='http:\/\/niu\/Kripo\/Employee\/detailEmployee\.aspx\?EmployeeId=e'/.test(kl) && /DienstNr=8123'/.test(kl) && /EmployeeNumberID=n'/.test(kl) && /memo_eingeben\.asp\?EmployeeID=e&EmployeeNumberID=n'/.test(kl) && kl.split(' | ').length === 11);
  process.exit(ok ? 0 : 1);
})();
